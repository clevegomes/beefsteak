...
class HydraAdminFormForm extends HydraAdminListFields {
    ...
    private function prepareData() {
        ...
        if ($records) {
            foreach ($records as $record) {
                $data[] = array(
                    'posttitle' => '<a href="' . $this->createRoute('form', 'list', array('post_type' => $record->name)) . '"><strong>' . $record->label . '</strong></a>',
                    'machinename' => $record->name,
                    'actions' => array(
                        'manage_edit' => $this->createLink(__('Edit form', 'hydraforms'), $this->createRoute('form', 'edit', array('post_type' => $record->name))),
                        'manage_delete' => $this->createLink(__('Delete form', 'hydraforms'), $this->createRoute('form', 'delete', array('post_type' => $record->name))),
                        'manage_field' => $this->createLink(__('List fields', 'hydraforms'), $this->createRoute('form', 'list', array('post_type' => $record->name))),
                        'manage_handlers' => $this->createLink(__('List handlers', 'hydraforms'), $this->createRoute('handler', 'list', array('post_type' => $record->name))),
                    ),
                );
            }
        }
    ...
    public function editForm($postType = NULL) {
        ...
        if (isset($record)) {
            ...
            // delete link
            $deleteLink = $this->createLink(__('Delete', 'hydraforms'), $this->createRoute('form', 'delete', array('post_type' => $postType)));
    ...
    public function addFormSubmit($form, $values) {
        ...
        $route = $this->createRoute('field', 'edit', array(
            'id' => $record->id,
