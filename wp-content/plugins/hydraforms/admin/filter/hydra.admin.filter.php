...
class HydraAdminFilterFilter extends HydraAdminListFields {
  ...
  private function prepareData() {
    ...
    if ($records) {
      foreach ($records as $record) {
        $data[] = array(
          'posttitle' => '<a href="' . $this->createRoute('filter', 'list', array('post_type' => $record->name)) . '"><strong>' . $record->label . '</strong></a>',
          'machinename' => $record->name,
          'actions' => array(
            'manage_edit' => $this->createLink(__('Edit filter', 'hydraforms'), $this->createRoute('filter', 'edit', array('post_type' => $record->name))),
            'manage_delete' => $this->createLink(__('Delete Filter', 'hydraforms'), $this->createRoute('filter', 'delete', array('id' => $record->id))),
            'manage_list' => $this->createLink(__('List fields', 'hydraforms'), $this->createRoute('filter', 'list', array('post_type' => $record->name))),
          ),
        );
      }
    }
  ...
  public function editForm($postType = NULL) {
    ...
    if(isset($record)) {
      // delete link
      $deleteLink = $this->createLink(__('Delete', 'hydraforms'), $this->createRoute('filter', 'delete', array('post_type' => $postType)));
  ...
  public function editFormSubmit($form, $values) {
    if (isset($values['post_type'])) {
      ...
      $form->addSuccessMessage(sprintf(__('Filter "%s" successfully updated', 'hydraforms'), $hydraFormRecord->getLabel()));
    }
    else {
      $hydraFormRecord = new HydraFormRecord($values);
      $form->addSuccessMessage(sprintf(__('Filter "%s" successfully created', 'hydraforms'), $hydraFormRecord->getLabel()));
    }
    ...
    $form->setRedirect($this->createRoute(
      'filter',
      'default', array()
    ));
  }
  ...
  public function addFormSubmit($form, $values) {
    ...
    $form->setRedirect($this->createRoute('field', 'edit', array(
      'id' => $field->id,
  ...
  public function deleteAction($id) {
    ...
    $form->addField('markup', array('message', __('Are you sure you want to delete this filter? This action can <b>not</b> reverted!', 'hydraforms')));
